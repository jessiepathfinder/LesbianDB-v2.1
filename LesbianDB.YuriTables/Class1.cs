	...
	public static class Extensions
	{
		...
		public static async Task<string> ArrayGetValue(this IOptimisticExecutionScope optimisticExecutionScope, string arrayname, BigInteger index){
			...
			if(end > index){
				...
				//Optimistic optimizations constraint: make sure that we are atomically consistent
				return optimisticExecutionScope is ISnapshotReadScope ? await optimisticExecutionScope.Read(key) : (await optimisticExecutionScope.VolatileRead(new string[] {key, arrayname2}))[key];
			}
			throw new IndexOutOfRangeException();
		}
		...
		public static async Task<ReadResult> ArrayTryGetValue(this IOptimisticExecutionScope optimisticExecutionScope, string arrayname, bool last)
		{
			...
			return new ReadResult(optimisticExecutionScope is ISnapshotReadScope ? await optimisticExecutionScope.Read(key) : (await optimisticExecutionScope.VolatileRead(new string[] { key, arrayname2 }))[key]);
		}
		...
		private static async IAsyncEnumerable<BigInteger> BTreeSelectAll(ISnapshotReadScope ISnapshotReadScope, string treename, bool reverse, JsonBTreeNode jsonBTreeNode){
			string read = await ISnapshotReadScope.Read(treename);
			if (read is null)
			{
				yield break;
			}
			...
			if(first is { }){
				await foreach(BigInteger bigInteger in BTreeSelectAll(ISnapshotReadScope, first, reverse, jsonBTreeNode)){
			...
			if (last is { })
			{
				await foreach (BigInteger bigInteger in BTreeSelectAll(ISnapshotReadScope, last, reverse, jsonBTreeNode))
				{
					yield return bigInteger;
				}
			}
